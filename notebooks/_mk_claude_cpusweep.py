...
def _reuse_predictions(method, stems):
    ...
    _reuse_inference_byproducts(source)


_REUSE_KEEP_OURS = {
    # Everything this run must compute for itself. Inheriting any of these would mean
    # reporting lb50's answers as ours, which is the one way this kernel could lie.
    "submission.csv", "ppsweep_results.csv", "ppsweep_selected.json",
    "validator_results.csv", "run_stats.csv",
}


def _reuse_inference_byproducts(source):
    """Copy the diagnostics the *inference* process writes, which we are not running.

    `retention_guard_*.jsonl` is the one the integrity cell actually reads -- it globs
    `/kaggle/working/retention_guard_*.jsonl` and raises "No frame-retention diagnostics
    were produced" on an empty list, which is exactly what skipping inference produces.
    The others are copied with it because they have the same provenance and none of them
    is regenerated by post-processing; the explicit keep-ours list above is what stops
    that generosity from importing lb50's conclusions.
    """
    ...
    for path in sorted(root.glob("*")):
        if not path.is_file() or path.name in _REUSE_KEEP_OURS:
            continue
        if path.suffix not in (".jsonl", ".json"):
            continue
        _cpu_shutil.copy2(path, Path("/kaggle/working") / path.name)
        copied.append(path.name)
    print(f"CPU SWEEP: reused {len(copied)} inference diagnostics from {root}: "
          f"{copied}", flush=True)'''
