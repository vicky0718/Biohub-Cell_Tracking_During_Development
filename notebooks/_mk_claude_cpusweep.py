...
def _reuse_predictions(method, stems):
    ...
    _reuse_inference_byproducts(source, method, want)


_REUSE_DIAG_PREFIXES = ("retention_guard_", "detector_coordinates_")


def _reuse_inference_byproducts(source, method, want):
    """Rebuild the per-stage diagnostics inference writes, filtered to THIS stage's movies.

    The integrity cell globs `/kaggle/working/retention_guard_*.jsonl` and requires the set
    of movies in them to equal the test clips exactly. v3 copied every diagnostic the mount
    had, which pulled in the *validator's* `retention_guard_single.jsonl` -- a file that, in
    the run being imitated, does not exist yet when that check runs, because the validator is
    a later cell. The check then saw twelve movies where it wanted four and refused.

    So rather than copy files, filter records: read every source `.jsonl`, keep the lines
    whose `dataset` belongs to the stems this stage is reusing, and write them to one
    stage-scoped file. Disjoint stems mean the duplicate check stays satisfied too, and the
    allowlist keeps Kaggle's own `__output__.json` -- which v3 also copied -- out of it.
    """
    root = next((p for p in source.parents
                 if list(p.glob("retention_guard_*.jsonl"))), None)
    if root is None:
        raise RuntimeError(
            f"CPU SWEEP: no retention_guard_*.jsonl above {source}; the integrity cell "
            f"will refuse the run and it is cheaper to say so here.")
    written = {}
    for prefix in _REUSE_DIAG_PREFIXES:
        rows = []
        for path in sorted(root.glob(prefix + "*.jsonl")):
            for line in path.read_text().splitlines():
                if not line.strip():
                    continue
                try:
                    record = _cpu_json.loads(line)
                except ValueError:
                    continue
                if str(record.get("dataset")) in want:
                    rows.append(line)
        if rows:
            out = Path("/kaggle/working") / f"{prefix}reuse_{method}.jsonl"
            out.write_text(chr(10).join(rows) + chr(10))
            written[out.name] = len(rows)
    if not written:
        raise RuntimeError(
            f"CPU SWEEP: {root} has no diagnostic records for {sorted(want)}; the "
            f"integrity cell would refuse the run seventy minutes from now.")
    print(f"CPU SWEEP: rebuilt diagnostics for {len(want)} movies: {written}", flush=True)'''
