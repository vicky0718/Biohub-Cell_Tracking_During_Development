"""Build notebooks/claude_submit_topk.ipynb — the 0.880 submission PLUS the ILP asymmetry.
...
OUT = Path("/workspace/biohub-cell_tracking_during_development/notebooks/claude_submit_topk.ipynb")
CELLS = []
...
WORKER.write_text(f'''
import inspect, json, sys, time, types
from pathlib import Path
import numpy as np
...
# THE CHANGE from the 0.899 submission — notes/40's best cell d0.975_m6_g2, +0.0036 on the
# same 12 train datasets where the 0.899 chain measured 0.9499. Three settings, all from
# the audit notes/39 made of the 0.927 public notebook's config against ours:
#   DET_THRESHOLD 0.99 -> 0.975  +0.0016 alone. notes/28 froze this at 0.99 so ONE
#                                leaderboard delta would isolate the repair, then never
#                                unfroze it. 0.99 is the WORST of the four swept, and the
#                                optimum at 0.975 is interior.
#   max_gap 1 -> 2               +0.0011. Their GAP_CLOSE_MAX_GAP is 2; ours was 1.
#   min_track 0 -> 6             +0.0009, but ONLY with max_gap=2. See repair_chain.
# det_threshold 0.999, and ONLY that -- every other setting is byte-identical to the
# chain that scored 0.901. claude_topk (n=36) measured 0.999 against the submitted
# 0.975: 28 of 36 datasets better, sign test p=0.0006, median +0.0065, 10% trimmed
# mean +0.0076 (t 3.92), +0.0099 (t 2.95) dropping the single -0.1189 outlier that
# holds the plain t to 1.31. On 6bba alone -- ~95% of the leaderboard by edge weight,
# per purescore's docstring -- it is +0.0085, t 2.60, 17 of 22 better.
#
# Never tested because notes/28 froze DET_THRESHOLD at 0.99 for one controlled
# comparison and every sweep since stayed inside [0.965, 0.99]. 0.999 sits just past
# that edge. Beyond it the detector collapses: 0.9999 is -0.176 and 0.999999 -0.858.
GAP_MAX_FRAMES, MIN_TRACK_FRAMES = 2, 6

def repair(g, scale):
    r = close_gaps(g.t, g.zyx, g.edges, scale=scale, max_um=GAP_UM,
                   max_added_frac=0.038, max_added_abs=1650, max_gap=GAP_MAX_FRAMES)
    r = linefit_smooth(*r, window=SMOOTH_WIN, weight=SMOOTH_W, scale=scale,
                       max_shift_um=SMOOTH_SHIFT)
    # notes/40: prune AFTER gap-closing, never before. Alone it is NEGATIVE (-0.0009);
    # it only pays once two-frame gaps have joined fragments into real tracks, so what
    # stays short is junk rather than an amputated piece of something real.
    r = prune_short_tracks(*r, min_frames=MIN_TRACK_FRAMES,
...
# THE ONE CHANGE from the 0.897 submission: bidirectional harmonic linking at w=0.15.
# notes/38: 0.9462 -> 0.9499 on 12 train datasets, +0.0036, above the 0.001 noise floor.
# notes/39 then confirmed this IS the operating point -- re-locating the ILP weights with
# the blend on adds only +0.0007, inside noise, because the two levers are substitutes.
#
# The mechanism is not the one the build predicted (notes/38 §1): fn_mislink falls by 3-7
# while fn_detect falls by 34-42. The blend admits ~2% more candidate edges, the ILP keeps
# more of them, and GT nodes previously stranded on dead track ends now match. Retention,
# not correction.
#
# Stated cost: the blend suppresses divisions (div_J 0.069 -> 0.057 at this weight), and
# notes/39 §2 established those forks are NOT recoverable by re-tuning -- the extra
# candidate edges remove the evidence for them, not just the incentive. w=0.15 is the point
# where the edge gain still outruns that loss; 0.25 and 0.40 give more edge and less total.
from pipeline.bidirectional import ANCHOR, harmonic_blend, patch_source
BIDIR_W = 0.15
_src = inspect.getsource(P.predict_video)
if ANCHOR not in _src:
    _i = _src.find("predict_edges")
    print("!! ANCHOR DOES NOT MATCH. predict_video near predict_edges:", flush=True)
    print(_src[max(0, _i - 500):_i + 1000], flush=True)
    raise SystemExit("anchor mismatch -- update pipeline/bidirectional.ANCHOR")
_ns = dict(P.__dict__)
exec(compile(patch_source(_src, BIDIR_W), "<bidir>", "exec"), _ns)
predict_video = _ns["predict_video"]
print(f"bidirectional blend ACTIVE at w={{BIDIR_W}} "
      f"(anchor matched {{_src.count(ANCHOR)}}x in {{len(_src):,}} chars)", flush=True)
...
def gen():
    ...
    for i, name in enumerate(names, 1):
        ...
        if starved or time.time() - t_start > TIME_BUDGET_S:
            ...
            continue
        t0 = time.time()
        coords, edges = predict_video(model, TEST / f"{{name}}.zarr", DEV, cfg=cfg,
